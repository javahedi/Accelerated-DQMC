...
int main() {
    ...
    std::vector<double> T_values = { 
        100.0, 40.0, 20.0, 10.0, 5.0, 2.0, 1.0, 0.5
    };
...
// struct ChainResult {
//     double density = 0.0;
//     double double_occ = 0.0;
//     double local_moment = 0.0;
// };

// ChainResult run_one_chain(
//     int chain_id,
//     int L_size,
//     double U,
//     double beta,
...
//     unsigned int chain_seed = base_seed + 10007u * static_cast<unsigned int>(chain_id);
...
//     InteractionConfig config(num_sites, num_slices, U, delta_tau);
//     config.randomize(chain_seed + 17u);

//     GreensEngine engine(num_sites, num_slices, k);
//     engine.recompute_all_from_scratch(config);

//     DqmcSampler sampler(num_sites, num_slices, k, chain_seed + 29u);
//     Measurement measurements(num_sites);
...
// Result run_one_simulation_parallel(
//     int L_size,
//     double U,
//     double beta,
//     double t,
...
//     int equilibration_per_chain =
//         std::max(1, total_equilibration_sweeps);

//     int measurement_per_chain =
//         std::max(1, total_measurement_sweeps / num_chains);

//     #pragma omp parallel for schedule(static)
//     for (int chain = 0; chain < num_chains; ++chain) {
//         chain_results[chain] = run_one_chain(
//             chain,
//             L_size,
//             U,
//             beta,
//             t,
//             target_delta_tau,
//             equilibration_per_chain,
//             measurement_per_chain,
//             reinversion_frequency,
//             base_seed
//         );
//     }
...
//     for (const auto& r : chain_results) {
//         density += r.density;
//         double_occ += r.double_occ;
//         local_moment += r.local_moment;
//     }

//     density /= static_cast<double>(num_chains);
//     double_occ /= static_cast<double>(num_chains);
//     local_moment /= static_cast<double>(num_chains);
...
//     //int reinversion_frequency = 1; // Recompute Green's functions from scratch every sweep for maximum accuracy
//     int reinversion_frequency = 3;

//     int num_chains = std::max(1, omp_get_max_threads());
...
//     std::cout << "Starting parallel local moment scan...\n";
//     std::cout << "Lattice: " << L_size << "x" << L_size << "\n";
//     std::cout << "t = " << t << "\n";
//     std::cout << "target_delta_tau = " << target_delta_tau << "\n";
//     std::cout << "OpenMP chains = " << num_chains << "\n";
...
//             Result r = run_one_simulation_parallel(
//                 L_size,
//                 U,
//                 beta,
//                 t,
